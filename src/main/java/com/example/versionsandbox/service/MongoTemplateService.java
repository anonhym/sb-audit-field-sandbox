...
public class MongoTemplateService {
    ...
    /**
     * The bulk back-fill migration: set {@code version = 0} on every document that has no
     * {@code version} field yet. {@code 0} is exactly what a native Spring Data insert stores, so
     * after this runs the previously-"legacy" documents load with a non-null version and a
     * subsequent repository {@code save()} takes the update path instead of failing.
     *
     * @return the number of documents migrated
     */
    public long backfillMissingVersions() {
        return mongoTemplate.getCollection(COLLECTION).updateMany(
