 ...
 * auditing does <strong>not</strong> cover (auditing only fires on {@code save()}).
 *
 ...
 * </ul>
 *
 * <p>{@code findAndReplace} (whole-document swap, which mis-handles the created fields) and
 * {@code bulkOps} (updates registered on a separate builder) cannot be fixed from a simple aspect —
 * they are logged at WARN. The recommended production setup pairs this aspect with an ArchUnit rule
 * that bans those two operations on audited entities (see the {@code archunit/*} branch).
 *
 * <p>The user for {@code @CreatedBy}/{@code @LastModifiedBy} comes from {@link CurrentUser}, the same
 * source the {@code save()}-path auditing uses.
 */
@Aspect
@Component
public class AuditFieldsAspect {
    ...
    public Object injectAuditFields(ProceedingJoinPoint pjp) throws Throwable {
        ...
        if (audited(entityType) && update instanceof Update u) {
            ...
            // $setOnInsert only takes effect when the operation inserts (upsert insert branch).
