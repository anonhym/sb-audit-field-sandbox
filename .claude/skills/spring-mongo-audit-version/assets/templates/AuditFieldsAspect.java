// TEMPLATE — copy into your project and adjust the package + the CurrentUser source.
// Requires AspectJ on the classpath. In Spring Boot 4 there is no spring-boot-starter-aop;
// add `org.aspectj:aspectjweaver` (AopAutoConfiguration enables @EnableAspectJAutoProxy when it is
// present). In Boot 3 you may instead use spring-boot-starter-aop.
...
public class AuditFieldsAspect {
    ...
    /**
     * The auditor for {@code @CreatedBy}/{@code @LastModifiedBy}. Wire this to the SAME source your
     * {@code AuditorAware} uses so the {@code save()} path and the template path agree (e.g. the
     * security context, or the request-scoped CurrentUser shown in the other template).
     */
